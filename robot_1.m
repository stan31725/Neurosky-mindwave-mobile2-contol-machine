...
newJoint.PositionLimits=[-0.5*pi,0.5*pi];
...
setFixedTransform(body7.Joint, trvec2tform([0.3, -0.4, 0.3]));
...
heightAboveTable.Bounds = [0, inf; ...
                           -inf, 0.4; ...
                           0.2, inf];
alignWithfood = constraintAiming('tool');
alignWithfood.TargetPoint = [-100, 0,100];

distanceFromfood = constraintPositionTarget('foodFrame');
distanceFromfood.ReferenceBody = 'tool';

distanceFromfood.PositionTolerance = 0.005;
...
for k = 3:numWaypoints
    ...
    distanceFromfood.TargetPosition(3) = distanceFromCupValues(k-1);
    ...
    [qWaypoints(k,:),solutionInfo] = gik(qWaypoints(k-1,:), ...
                                         heightAboveTable, ...
                                         distanceFromfood, alignWithfood, ...
...
heightAboveTable.Bounds = [0, inf; ...
                           ...
                           0.2, inf];
alignWithfood = constraintAiming('tool');
alignWithfood.TargetPoint = [0, -100,0];

distanceFromfood = constraintPositionTarget('mouthFrame');
distanceFromfood.ReferenceBody = 'tool';
